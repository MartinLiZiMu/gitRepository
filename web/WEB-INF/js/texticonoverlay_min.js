...
(function(){
		...
		c.dom.g=function(f){
			...
			c.dom._g=function(f){
				if(c.lang.isString(f)){
					return document.getElementById(f)}return f
				};
			c._g=c.dom._g;
			c.browser=c.browser||{};
			...
			var i=c.dom.getDocument(g),h;
				if(i.defaultView&&i.defaultView.getComputedStyle){
					...
					if(h){
						return h[f]||h.getPropertyValue(f)}}return""
					};
			...
			c.dom.getStyle=function(h,g){
				...
				c.dom.getPosition=function(f){
					f=c.dom.g(f);
					var o=c.dom.getDocument(f),i=c.browser,l=c.dom.getStyle,h=i.isGecko>0&&o.getBoxObjectFor&&l(f,"position")=="absolute"&&(f.style.top===""||f.style.left===""),m={left:0,top:0},k=(i.ie&&!i.isStrict)?o.body:o.documentElement,p,g;
					if(f==k){
						return m
						}
					if(f.getBoundingClientRect){
						...
						m.left-=o.documentElement.clientLeft;m.top-=o.documentElement.clientTop;
						var n=o.body,q=parseInt(l(n,"borderLeftWidth")),j=parseInt(l(n,"borderTopWidth"));
						if(i.ie&&!i.isStrict){
							m.left-=isNaN(q)?2:q;m.top-=isNaN(j)?2:j
									}
						}else{
							p=f;
							do{
								m.left+=p.offsetLeft;m.top+=p.offsetTop;
							...
							if(i.opera>0||(i.isWebkit>0&&l(f,"position")=="absolute")){
								m.top-=o.body.offsetTop}p=f.offsetParent;
								while(p&&p!=o.body){
									m.left-=p.scrollLeft;
	...
	(function(){
		...
		c.lang.Class.prototype.dispatchEvent=function(j,f){
			...
			f=f||{};
			for(var h in f){
				j[h]=f[h]}var h,g=this.__listeners,k=j.type;
		...
		c.lang.inherits=function(l,j,i){
			...
			k=l.prototype=new g();
			for(h in f){
				k[h]=f[h]}l.prototype.constructor=l;
