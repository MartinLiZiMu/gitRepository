...
(function(){
	...
	var f=function(j,k,h){k&&(j=Math.max(j,k));
	h&&(j=Math.min(j,h));
	return j};
	
	var a=function(h){return"[object Array]"===Object.prototype.toString.call(h)};
	var c=function(l,n){
		var j=-1;
		if(a(n)){if(n.indexOf){j=n.indexOf(l)
			}else{
	...
	var j=this;this._map.addEventListener("zoomend",function(){
		j._redraw()});
		this._map.addEventListener("moveend",function(){
			j._redraw()});
		var i=k.markers;a(i)&&this.addMarkers(i)
		};
		...
		e.prototype.addMarker=function(h){
			this._pushMarkerTo(h);this._createClusters()
			};
		...
		e.prototype.setMaxZoom=function(h){
			this._maxZoom=h;this._redraw()
			};
		e.prototype.getStyles=function(){
			return this._styles
			};
		e.prototype.setStyles=function(h){
			this._styles=h;this._redraw()
			};
		...
		g.prototype.updateClusterMarker=function(){
			...
			var k=this._map;var h=this.getBounds();
			this._clusterMarker.addEventListener("click",function(i){k.setViewport(h)})
				};
